#!/usr/bin/env bash
# Since Gitlab's GraphQL schema is quite large by default and we only use a small subset,
# this script filters the json to only include what we need.
# It's automatically called in the Justfile, after downloading a new version of the schema from Gitlab.
#
# Usage: prune-graphql-schema.sh path-to-file
