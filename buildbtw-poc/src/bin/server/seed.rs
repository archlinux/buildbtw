...
/// Some arbitrarily chosen packages.
const PACKAGES: &[&str] = &[
    "llvm",
    "eza",
    "zenity",
...
/// Insert lots of data into the server's database for testing.
pub async fn seed(pool: &sqlx::SqlitePool) -> Result<()> {
    match db::namespace::read_latest(pool).await {
        Ok(_) => {
            return Err(eyre!(
                "Database already contains previous namespaces. Please wipe your database and re-run this script."
            ));
        }
