...
use buildbtw::db_fields::RedactedString;
use buildbtw::{api, from_request, web};

use crate::test_ctx::{CookieJarExt, TestCtx, ctx};
...
async fn test_authenticate_with_cookie(#[future(awt)] ctx: TestCtx) -> Result<()> {
    ...
    let private_jar = from_request::auth_user::save_in_cookie_jar(&secret_token, private_jar, None);
...
async fn test_cookie_and_bearer_token_invalid(#[future(awt)] ctx: TestCtx) -> Result<()> {
    ...
    let private_jar = ctx.private_cookie_jar();
    let private_jar =
        from_request::auth_user::save_in_cookie_jar(&invalid_secret_token, private_jar, None);
    let cookies = private_jar.to_encrypted_cookie_jar()?;
