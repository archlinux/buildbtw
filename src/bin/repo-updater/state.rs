...
impl State {
    ...
    pub fn get_path_updated(&self, path: &Utf8Path) -> Result<Option<&OffsetDateTime>> {
        let absolute_path = path.canonicalize()?;
        Ok(self.last_updated.get(&absolute_path))
    }

    pub fn set_path_updated(&mut self, path: &Utf8Path, updated: OffsetDateTime) -> Result<()> {
        let absolute_path = path.canonicalize()?;
    ...
    fn state_file() -> Result<PathBuf> {
        Ok(Self::state_dir()?.join("repo_updater_state.toml"))
    }
}
