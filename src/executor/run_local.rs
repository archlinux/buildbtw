...
async fn try_build(
    ...
    tracing::debug!(?build.pkgbase, ?build.architecture, "Running build");
    ...
    let buildspace = iteration.buildspace.clone().into_option().ok_or_eyre("")?;
    let log_file =
...
async fn copy_build_artifacts(
    ...
    tracing::debug!("Moving artifacts to server data dir...");
    ...
    for (source_path, target_path) in &move_ops {
        tracing::trace!(?source_path, ?target_path, "Moving file");
        fs::copy(source_path, target_path).await?;
    }

    tracing::debug!("Moved {} files to server data dir", move_ops.len());
