...
pub fn build_artifact_path(
    ...
    let dest_dir = build_repo_path(build, override_data_dir)?;
...
pub fn build_repo_path(
    ...
    let dest_dir = build_iteration_arch_dir(build, override_data_dir)?.join("repo");
...
pub fn build_log_path(
    ...
    let dest_dir = build_iteration_arch_dir(build, override_data_dir)?.join("logs");
...
pub fn build_iteration_arch_dir(
    ...
    let dest_dir = artifact_storage_base_path
        .join("buildspace")
        .join(build.iteration.buildspace.name.to_string())
        .join("iteration")
        .join(build.iteration.sequence.to_string())
        .join("os")
...
pub fn build_fully_uploaded(
    ...
    let filenames = &build.pkgnames_filenames.0;
    filenames.keys().all(
        |pkgname| match build_artifact_path(build, pkgname, override_data_dir) {
            Ok(path) => path.exists(),
            _ => false,
        },
    )
