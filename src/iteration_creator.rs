...
impl IterationCreator {
    ...
    ///
    /// Returns an error if `iteration.status` is not [`entities::iterations::Status::PendingCalculation`].
    async fn calculate_pending_build_graph(
        &self,
        source_repos: &mut dependency_graph::SourceRepoCache,
        iteration: &entities::iterations::Model,
    ) -> Result<()> {
        if iteration.status != entities::iterations::Status::PendingCalculation {
            bail!(
                "Refusing to calculate build graph for iteration {} in status {:?}",
                iteration.id,
                iteration.status
            );
        }
