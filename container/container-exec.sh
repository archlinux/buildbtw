...
# We inject the GitLab host key into /root/.ssh/known_hosts.
...
echo "$BUILDBTW_GITLAB_SSH_HOST_KEY" > /root/.ssh/known_hosts

gitlab_domain=$(echo "${BUILDBTW_GITLAB_DOMAIN}" | sed -E 's|^https?://||')
if ! ssh-keygen -f /root/.ssh/known_hosts -F "${gitlab_domain}"; then
...
if ! ssh-keygen -l -f /root/.ssh/known_hosts; then
    echo 'ERROR: BUILDBTW_GITLAB_SSH_HOST_KEY is not a valid ssh_known_hosts entry' >&2
    exit 1
fi

if [[ ! -f /root/.ssh/id_ed25519 ]]; then
    echo 'ERROR: /root/.ssh/id_ed25519 must exist' >&2
    exit 1
fi
eval "$(ssh-agent -s)"
